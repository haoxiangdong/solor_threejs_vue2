...
export default {
  ...
  methods: {
    ...
    // 创建网格模型
    createMesh() {
      const geometry = new THREE.SphereGeometry(-1500, 200, 10, 10) // 创建一个平面对象PlaneGeometry
      var backgroundImgTexture = new THREE.TextureLoader().load(backgroundImg)
      const planeMaterial = new THREE.MeshLambertMaterial({
        color: red,
        map:backgroundImgTexture
      }) // 材质对象Material
      ...
      atmosphereMeshMaterial.side = THREE.DoubleSide
