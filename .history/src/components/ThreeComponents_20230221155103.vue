...
export default {
  ...
  methods: {
    ...
    createMercuryMesh() {
      const geometry = new THREE.SphereGeometry(12.5, 200, 200) 
      ...
      // 设置位置
      plane.rotation.x = -0.5 * Math.PI
      plane.position.set(130, 0, 0)
      // plane.
      // 平面对象添加到场景中
      this.scene.add(plane)
    },
    createVenusMesh() {
      const geometry = new THREE.SphereGeometry(21, 200, 200) 
    ...
    createCamera() {
      ...
      this.camera.position.set(500, 1500, 15000) // 设置相机位置
