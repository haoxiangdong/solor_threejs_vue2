...
export default {
  ...
  methods: {
    ...
    createMesh(){
      ...
      const geometrys = new THREE.SphereGeometry(200, 200, 10, 10) // 创建一个平面对象PlaneGeometry
      const planeMaterial = new THREE.MeshLambertMaterial({
        color: 0x777777
      }) // 材质对象Material
      const shareMaterial = new THREE.MeshLambertMaterial({
        color: 0x777777
      }) // 材质对象Material
      const plane = new THREE.Mesh(geometry, planeMaterial)
      const share = new THREE.Mesh(geometrys, shareMaterial)
